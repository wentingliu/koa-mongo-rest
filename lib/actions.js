...
module.exports = function(model) {
  return {
    findAll: function*() {
      var error, result;
      try {
        params = parseRaw(this.request.query, model);
        result = yield model.find(params.searchParams).limit(params.per_page).skip((params.page - 1) * params.per_page).exec();
...
function parseRaw(rawParams, model) {
